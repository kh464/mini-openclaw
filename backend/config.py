...
config = load_config()
"""
    单例模式: 这一行在模块加载时立即执行 load_config()。由于 Python 模块导入的特性，其他模块（如 app.py 或 agent.py）通过 from backend.config import config 导入的将是同一个 AppConfig 实例。
    这种做法保证了配置在内存中的唯一性，修改 config 的属性后调用 save_config(config) 即可同步更新磁盘文件。
"""